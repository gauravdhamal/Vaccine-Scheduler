...
public interface SlotRepo extends JpaRepository<Slot, Integer> {
    @Query(value = "SELECT s.* FROM slot s LEFT JOIN vaccine v ON s.vaccine_vaccine_id = v.vaccine_id WHERE v.vaccine_name = :vaccineName AND ((s.start_time > :currentTime AND s.slot_date = :currentDateEqual) OR s.slot_date > :currentDate)", nativeQuery = true)
